...
public class SnakeGame implements ActionListener, MouseListener
{
	...
	private void spawnFood()
	{
		int tempx;
		int tempy;
		// tempx und tempy werden so lange neu ausgewürfelt bis sie nicht mehr auf einer
		// Schlange oder Barrikade sind
		do
		{
			tempx = (int) (Math.random() * numColumns);
			tempy = (int) (Math.random() * numRows);
		}
		while (barrier.isOccupied(tempx, tempy) || snake.isOccupied(tempx, tempy));
	...
	private void spawnPowerUp()
	{
		...
		while (barrier.isOccupied(tempx, tempy) || snake.isOccupied(tempx, tempy) || food.isOccupied(tempx, tempy));

		powerUp = new PowerUp(tempx, tempy);
	}
	...
	private void nextStep()
	{
		...
		if (snake.isAlive())
		{
			...
			case 0: // Up
				...
				try
				{
					snake.img = ImageIO.read(new File("./src/textures/snakeHeadUp.png"));
				}
				catch (IOException e)
				{
			...
			case 1: // Right
				...
				try
				{
					snake.img = ImageIO.read(new File("./src/textures/snakeHeadRight.png"));
				}
				catch (IOException e)
				{
					System.out.println("keine Textur");
			...
			case 2: // Down
				...
				try
				{
					snake.img = ImageIO.read(new File("./src/textures/snakeHeadDown.png"));
				}
				catch (IOException e)
				{
					System.out.println("keine Textur");
			...
			case 3: // Left
				...
				try
				{
					snake.img = ImageIO.read(new File("./src/textures/snakeHeadLeft.png"));
				}
				catch (IOException e)
				{
					System.out.println("keine Textur");
			...
			if (snake.move(food, barrier))
			{
				...
				if (foodEaten % 3 == 0)
				{
					spawnPowerUp();
					timer.setDelay(currentGameSpeed);
					barrier.regularTexture();
					snake.rainbowMode = false;
					snake.regularTexture();
					snake.doubleMode = false;
				}
			}

			if (powerUp.isOccupied(snake.getPos().x, snake.getPos().y))
			{
				switch (powerUp.getValue())
				{
				case 0: // slow - move slower
					timer.setDelay(currentGameSpeed * 2);
					break;
				case 1: // double food Value
					snake.doubleMode = true;
					break;
				case 2: // RAINBOW - no Collision & fast Speed
					barrier.rainbowTexture();
					snake.rainbowMode = true;
					timer.setDelay(currentGameSpeed/2);
					snake.rainbowTexture();
				}
				powerUp = new PowerUp(-5, -5); // powerUp verstecken bis ein neues erscheint
			}
		}
	...
	{
		return new GameScore(new String[]
		{ "Snake Length", "Food eaten", "Lives left" }, new int[]
		{ snake.getLength(), foodEaten, snake.getLives() }, new int[]
		{ SNAKE_LENGTH_GOAL * level, FOOD_ITEM_GOAL * level, snakeLives });
	}
	...
	public void paintGameArea(Graphics2D g)
	{
		...
		powerUp.draw(g, snakeArea, tileSize);
	}
