...
public class Node implements SortedFloatTree
{
	...
	public boolean exists(float element)
	{
		...
		return true;
		//return (this.element == element || left.exists(element) || right.exists(element));
	}
	...
	{
		if (element < this.element)
			return new Node(this.element, left.insert(element), right);
		if (element > this.element)
			return new Node(this.element, left, right.insert(element));
		return this;
	}
}
